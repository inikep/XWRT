...
void CDecoder::SetDictionarySize(int powerOfTwo)
{
    ...
    ICompressProgressInfo *progress)
{
  ...
  while (true)
  {
    UInt32 curSize = (UInt64) outSize; // 1 << 18
  ...
  UInt64 u=_rangeDecoder.Stream.GetProcessedSize();
  outSize=(UInt64 *)&u;
...
UInt32 CDecoder::GetFilePos()
{
	return (UInt32) _rangeDecoder.Stream.GetProcessedSize();
}
      ...
      ICompressProgressInfo *progress)
{
  ...
  if (!_literalDecoder.Create(_numLiteralPosStateBits, _numLiteralContextBits))
    return E_OUTOFMEMORY;
